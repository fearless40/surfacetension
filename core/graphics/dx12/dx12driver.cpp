...
void D3D12Driver::CreateDeviceResources()
{
#if defined(_DEBUG)
    ...
    m_d3dDevice->SetName(L"D3D12Driver");
    ...
    m_commandQueue->SetName(L"D3D12Driver");
    ...
    m_rtvDescriptorHeap->SetName(L"D3D12Driver");
    ...
    if (m_depthBufferFormat != DXGI_FORMAT_UNKNOWN)
    {
        ...
        m_dsvDescriptorHeap->SetName(L"D3D12Driver");
    }
    ...
    m_commandList->SetName(L"D3D12Driver");
...
void D3D12Driver::CreateWindowSizeDependentResources()
{
    if (!m_window)
    {
...
void D3D12Driver::SetWindow(HWND window, int width, int height) noexcept
{
    m_window = window;
...
bool D3D12Driver::WindowSizeChanged(int width, int height)
{
    if (!m_window)
        return false;
...
void D3D12Driver::Prepare(D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState)
{
...
void D3D12Driver::Present(D3D12_RESOURCE_STATES beforeState)
{
...
void D3D12Driver::WaitForGpu() noexcept
{
...
void D3D12Driver::MoveToNextFrame()
{
...
void D3D12Driver::GetAdapter(IDXGIAdapter1 **ppAdapter)
{
    *ppAdapter = nullptr;
...
void D3D12Driver::UpdateColorSpace()
{
    if (!m_dxgiFactory)
        return;
